...
class ArgumentParser: 
    def __init__(self):
        ...
        self._parser.add_argument('--alpha', type=float, default=0.05, help='Learning rate')
        ...
        self._parser.add_argument('--iterations', type=int, default=1, help='Number of iterations')
        self._parser.add_argument('--lambda_value', type=int, default=1000, help='Lambda parameter')
        self._parser.add_argument('--sampling_strategy', type=int, default=1, help='Sampling strategy')
        
        self._parser.add_argument('--play_episodes', type=int, default=500, help='Number of episodes to play')
...
def run_simu_dice(dataset_path, max_training_episodes, environment_name, alpha, gamma, planning_steps, iterations, lamba, sampling_strategy,
    ...
    for seed in seeds:
        ...
        agent = SimuDICE(env, EpsilonGreedyPolicy(0.0), TabularWorldModel(action_number=env.get_action_number(), state_number=env.get_state_number(), seed=seed), 
        ...
        avg_reward, _ = agent.play(play_episodes, max_environment_steps, False, debug, seed)
        rewards.append(avg_reward)
    
    rewards = [float(r) for r in rewards]
    print('Average reward: {0}, with min reward {1} and max reward {2}'.format(np.mean(rewards), np.min(rewards), np.max(rewards)))
